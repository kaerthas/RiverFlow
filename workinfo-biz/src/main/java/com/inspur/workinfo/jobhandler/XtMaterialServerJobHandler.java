    ...
    public ReturnT<String> execute(String s) throws Exception {
        ...
        if (StrUtil.isBlank(xt_business_info)) {
            ...
            try {
                ...
                int XtApproveBusinessCoursesCount = courseService.count(queryWrapper);
                int XtApproveBusinessCoursesPageCount = XtApproveBusinessCoursesCount/limit;
                for(int k = 1;k<XtApproveBusinessCoursesPageCount+2;k++) {
                    Page page = new Page(k, limit);
                    //1获取流程为材料类型的环节信息
